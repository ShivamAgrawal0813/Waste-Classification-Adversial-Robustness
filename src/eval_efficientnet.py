"""
Evaluation script for EfficientNet-V2-S.
...
from src.config import (
    NUM_CLASSES, EFFNET_CLEAN_PATH, EFFNET_METRICS_FILE,
    EPS_FGSM, EPS_PGD, PGD_ALPHA, PGD_STEPS_EVAL,
    ACCURACY_VS_EPS_EPSILONS, OUTPUTS_DIR,
    EFFNET_CONFUSION_CLEAN, EFFNET_CONFUSION_PGD, EFFNET_ACC_VS_EPS
)
...
def evaluate_accuracy_vs_epsilon(model: nn.Module, test_loader, device: torch.device,
                                 epsilons: list, model_name: str = "EfficientNet") -> dict:
...
def evaluate():
    ...
    print(f"Loading {MODEL_NAME} model from {EFFNET_CLEAN_PATH}...")
    model = get_model(MODEL_NAME, NUM_CLASSES, pretrained=False)
    
    if not os.path.exists(EFFNET_CLEAN_PATH):
        print(f"Error: Model not found at {EFFNET_CLEAN_PATH}. Please train it first.")
        return
        
    checkpoint = torch.load(EFFNET_CLEAN_PATH, map_location=device)
    ...
    plot_confusion_matrix(
        ...
        EFFNET_CONFUSION_CLEAN,
        f"{MODEL_NAME} - Clean Confusion Matrix"
    )
    ...
    plot_confusion_matrix(
        ...
        EFFNET_CONFUSION_PGD,
        f"{MODEL_NAME} - PGD Attack Confusion Matrix"
    )
    ...
    metrics = {}
    if os.path.exists(EFFNET_METRICS_FILE):
        with open(EFFNET_METRICS_FILE, 'r') as f:
            metrics = json.load(f)
    ...
    # Save metrics
    with open(EFFNET_METRICS_FILE, 'w') as f:
        json.dump(metrics, f, indent=4)
        
    print(f"\nAll metrics and plots updated and saved.")
    print(f"Metrics: {EFFNET_METRICS_FILE}")
    print(f"Plots: {EFFNET_CONFUSION_CLEAN}, {EFFNET_CONFUSION_PGD}, {EFFNET_ACC_VS_EPS}")
