"""
Training script for EfficientNet-V2-S clean model on TrashNet.
Custom parameters: image_size=128, batch_size=16, epochs=40, num_workers=2.
"""
...
from src.config import (
    LR, MOMENTUM, WEIGHT_DECAY, NUM_CLASSES, EFFNET_CLEAN_PATH, 
    EFFNET_METRICS_FILE
)
...
def train():
    ...
    for epoch in range(EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            save_model(model, EFFNET_CLEAN_PATH, epoch=epoch, optimizer=optimizer,
    ...
    print(f"\nModel saved to {EFFNET_CLEAN_PATH}")
    print(f"Metrics saved to {EFFNET_METRICS_FILE}")
